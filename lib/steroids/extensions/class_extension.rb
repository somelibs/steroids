    ...
    module ClassExtension
      ...
      def attribute(attribute_name, default: nil, type: nil, allow_nil: false)
        ...
        define_method(attribute_name) do
          unless instance_variable_get(instance_variable_name) || has_been_set || frozen?
        ...
        # Define writter
        # - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
        define_method(:"#{attribute_name}=") do |next_value|
      ...
      def forward_methods_to(method_name, **options)
        ...
        return unless instance_methods.include?(method_name) && respond_to_hander.present?

        define_method(:respond_to_missing?) do |missing_method_name, include_private = false|
          !!(send(respond_to_hander, missing_method_name) || super(missing_method_name, include_private))
        end

        define_method(:method_missing) do |missing_method_name, *arguments, **options, &block|
          if send(respond_to_hander, missing_method_name)
            send_apply(method_name, missing_method_name)
          else
      ...
      def try_delegate(*method_names, **options)
        ...
        method_names.each do |method_name|
          define_method(method_name) do |*arguments, **options, &block|
            delegate_object = begin
              try(delegate_name)
            rescue
              false
            end
      ...
      def proxy(instance, &block)
        ...
        Class.build_anonymous(name) do
          ...
          define_method(:respond_to_missing?) do |missing_method_name, *_arguments, **_options|
