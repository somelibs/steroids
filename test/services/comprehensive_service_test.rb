...
class ComprehensiveServiceTest < ActiveSupport::TestCase
  ...
  def setup
    @original_wrap = begin
      Steroids::Services::Base.class_variable_get(:@@wrap_in_transaction)
    rescue
      true
    end
  ...
  test "service with block yields service instance for status checking" do
    ...
    CreateUserService.call(name: "Jane", email: "jane@example.com") do |_service, _outcome, **options|
  ...
  test "service with errors in block pattern" do
    ...
    CreateUserService.call(name: "Jane", email: "") do |_service, _outcome, **options|
      noticable = options[:noticable]
